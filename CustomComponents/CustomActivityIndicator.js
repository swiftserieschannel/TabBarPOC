...
export default class CustomActivityIndicator extends PureComponent {
  ...
  renderComponent({ index, count, progress }) {
    ...
    let frames = 60 * animationDuration / 600;
    let easing = Easing.bezier(0.4, 0.0, 0.7, 1.0);

    let inputRange = Array.from(new Array(frames), (undefined, frameIndex) => frameIndex / (frames - 1));

    let outputRange = Array.from(new Array(frames), (undefined, frameIndex) => {
      let progress = 2 * frameIndex / (frames - 1);
      let rotation = index ? +(360 - 15) : -(180 - 15);
